function [X,prom,desv,mruta,mcosto,mb] = plotRecocidotw(p,n,varargin)
...
mruta = res.x{end-2};    % modificacion para guardar la mejor ruta
mcosto = sum(res.x{end-1}); % modificacion para guardar el mejor costo
X = res.intentos;
Y = res.f;
nx = length(X);
for i=2:n
